...
const CadastroCliente = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(files).forEach(([key, file]) => {
      data.append('arquivos', file); // todos vão no mesmo array no backend
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto p-4 bg-white rounded-2xl shadow-md mt-4">
      ...
      <div className="space-y-4">
        ...
        <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-xl mt-6 font-semibold">
          Cadastrar
        </button>
